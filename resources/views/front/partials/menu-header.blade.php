...
<ul class="navigation">
    @foreach($parentMenus as $menu)
        <li class="{{ $menus->where('parent_id', $menu->id)->count() > 0 ? 'menu-item-has-children' : '' }}">
            <a href="{{ $menu->link }}">{{ $menu->name }}</a>

            @if($menus->where('parent_id', $menu->id)->count() > 0)
                <ul class="sub-menu">
                    @foreach($menus->where('parent_id', $menu->id) as $submenu)
                        <li class="{{ $menus->where('parent_id', $submenu->id)->count() > 0 ? 'menu-item-has-children menu-arrow-left' : '' }}">
                            ...
                            @if($menus->where('parent_id', $submenu->id)->count() > 0)
                                <ul class="sub-menu">
                                    @foreach($menus->where('parent_id', $submenu->id) as $thirdLevel)
                                        <li><a href="{{ $thirdLevel->link }}">{{ $thirdLevel->name }}</a></li>
                                    @endforeach
                                </ul>
                            @endif
                        </li>
